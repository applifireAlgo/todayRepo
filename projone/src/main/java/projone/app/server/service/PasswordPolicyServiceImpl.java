...
import projone.app.server.repository.PasswordPolicyRepository;
...
@SourceCodeAuthorClass(createdBy = "a@y.co", updatedBy = "", versionNumber = "1", comments = "Service for PasswordPolicy Master table Entity", complexity = Complexity.LOW)
...
public class PasswordPolicyServiceImpl extends PasswordPolicyService {
    ...
    public HttpEntity<ResponseBean> findById(@RequestBody FindByBean findByBean) throws SpartanPersistenceException, Exception {
        ...
        org.springframework.http.HttpStatus httpStatus = org.springframework.http.HttpStatus.OK;
        try {
            projone.app.shared.authentication.PasswordPolicy lstpasswordpolicy = passwordPolicyrepo.findById((java.lang.String) findByBean.getFindKey());
            responseBean.add("success", true);
            responseBean.add("message", "Successfully retrived ");
            responseBean.add("data", lstpasswordpolicy);
        } catch (org.springframework.transaction.TransactionException e) {
